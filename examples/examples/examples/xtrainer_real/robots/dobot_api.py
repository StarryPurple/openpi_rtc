...
import os
import json
# from examples.xtrainer_real.scripts.function_util import log_write
...
class DobotApi:
    def __init__(self, ip, port, *args):
        ...
        if self.port == 29999 or self.port == 30004:
            ...
            self.socket_dobot = socket.socket()
            self.socket_dobot.settimeout(0.1)
            self.socket_dobot.connect((self.ip, self.port))
        else:
            raise Exception(
...
class DobotApiMove(DobotApi):
    ...
    def ServoJ(self, j1, j2, j3, j4, j5, j6, t=0.1):
        ...
        """
        string = "ServoJ({:f},{:f},{:f},{:f},{:f},{:f},t={:f},gain=500)".format(
            j1, j2, j3, j4, j5, j6, t)
        # log_write(__file__, self.ip + "servoj: " + string)
