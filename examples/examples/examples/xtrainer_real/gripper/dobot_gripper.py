...
from examples.xtrainer_real.scripts.function_util import save_videos, mk_dir, free_limit_and_set_one
...
def main():
    # gripper = DobotGripper(port="/dev/ttyUSB0", servo_pos=[2048, 3052], id_name=22)
    gripper = DobotGripper(port="/dev/ttyUSB1", servo_pos=[2048, 1024], id_name=21)
    for i in [10, 50, 100, 150, 254]:
        gripper.move(i, 100, 1)
        time.sleep(1)
        gripper.move(i, 100, 1)
