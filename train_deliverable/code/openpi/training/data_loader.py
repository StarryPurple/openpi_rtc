...
def create_torch_dataset(
    ...
    if data_config.slow_channel_delay_max > 0:
        # πR² slow channel: also fetch past frames so the pipeline can select
        # a stale observation (images + state) for the cached prefix. The last
        # index of each returned tensor is the current frame.
        for key in data_config.slow_channel_keys:
            delta_timestamps[key] = [
                t / dataset_meta.fps
                for t in range(-data_config.slow_channel_delay_max, 1)
            ]
